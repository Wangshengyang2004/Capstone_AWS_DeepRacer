...
fpath = "output/optimized_waypoints.npy"
...
print("Zero velocity indices:", [i for i, v in enumerate(velocity) if v == 0])
    
time_to_prev = [(distance_to_prev[i]/velocity[i]) if velocity[i] != 0 else 0 for i in range(len(racing_track))]
...
print("Zero radius indices:", [i for i, r in enumerate(radius_direction) if r == 0])
steering = []
for i in range(len(racing_track)):
    if radius_direction[i] != 0:
        steer = math.degrees(math.asin(dist_wheels_front_back / radius_direction[i]))
    else:
        steer = 0  # or some other appropriate value
